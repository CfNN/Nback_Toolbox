...
if settings.UseMRITrigger
    % The trigger device is a keyboard. Loop through keyboards until you
    % find one with a vendor ID that matches the trigger device. For MRI
    % trigger use 'Current Designs, Inc.'
    trigger=-1;
...
timedout = false;
    while ~timedout
        ...
        % Uncomment these lines if you want the session to start
        % automatically without the trigger after a certain time (in
        % seconds)
